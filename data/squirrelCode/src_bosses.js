...
Boss =  ((function(){ let squirrelClassFunction; squirrelClassFunction = function ( ) { var returnVal = { constructor: function(){} } ;  returnVal = Enemy ( 'DO_NOT_CALL_CONSTRUCTOR' ) ; var baseMethods = { ... returnVal }; var baseConstructor = returnVal.constructor;  for (var baseProperty in returnVal) { 
 ...
 returnVal.constructor(...arguments); returnVal.SQUIRREL_CLASS = squirrelClassFunction; return returnVal ;  };  squirrelClassFunction . health = 40 * 30 ; 
...
deleteActor ( id )  ; 
 if ( mapActor . rawin ( doorID )  )  if ( actor [ mapActor [ doorID ]  ]  . rawin ( "opening" )  ) actor [ mapActor [ doorID ]  ]  . opening = true ; 
  ...
  if ( getFrames (  )  % 4 == 0 )  { 
  if ( health < healthActual - 16 ) health += 16 ; 
 
  if ( health < healthActual - 8 ) health += 8 ; 
 
  if ( health < healthActual - 4 ) health += 4 ; 
 
  if ( health < healthActual - 2 ) health += 2 ; 
 
  if ( health > healthActual + 80 ) health -= 80 ; 
 
  if ( health > healthActual + 40 ) health -= 40 ; 
 
  if ( health > healthActual + 20 ) health -= 20 ; 
 
  if ( health > healthActual + 10 ) health -= 10 ; 
...
vspeed = 0.0 ; 
 ...
 anim = anJump ; 
frame = anim [ 0 ]  ; 
newActor ( YetiShock , x , y + 32 ,  ( 40 - health / 30 )  / 20.0 + 1.0 )  ; 
 } 
...
vspeed =  - 2.0 ; 
 ...
 squirrelClassFunction . health = 40 * 30 ; 
