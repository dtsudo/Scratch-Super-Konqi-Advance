...
nowInWater = inWater ( x , y )  ; 
 ...
 if ( anim == "ball" ) friction = 0.025 ; 
  ...
  if (  ( placeFree ( x , y + 2 )  || vspeed < 0 )  &&  ( vspeed < 2 ||  ( vspeed < 16 &&  ! nowInWater )  )  && antigrav <= 0 ) vspeed +=  ( vspeed > 5 && anim != "stomp" ? gravity / vspeed : gravity )  ; 
...
canJump = 0 ; 
 ...
 frame = 0.0 ; 
  ...
  if ( abs ( hspeed )  > 2 ) animOffset = 8.0 ; 
 
  if ( abs ( hspeed )  > 4 ) animOffset = 16.0 ; 
 
  if ( abs ( hspeed )  > 6.2 ) animOffset = 24.0 ; 
...
accel *= 2.0 ; 
  ...
  if ( hspeed > 1 ) hspeed += accel / fabs ( hspeed )  ; 
...
c . vspeed =  - 0.5 + randFloat ( 1 )  ; 
 ...
 damageMultF . fire = 0.0 ; 
  ...
  var godir = pointAngle ( x , y - 16 , actor [ homingTarget ]  . x , actor [ homingTarget ]  . y )  ;
...
didAirSpecial = false ; 
 ...
 var choffset = 0 ;
  ...
  else drawSpriteZ ( 2 , sprite , an [ anim ]  [ floor ( frame )  ]  + animOffset , x - camx , y - 2 - camy + yoff , walkAngle , flip , 1 , 1 ,  ( blinking && anim != "hurt" ? wrap ( blinking , 0 , 10 )  . tofloat (  )  / 10.0 : 1 )  )  ; 
