...
drawRec ( 0 , 0 , gvScreenW , gvScreenH , true )  ; 
 ...
 break ;  default : drawSprite ( getroottable (  )  [ gvCharacters [ game . playerChar ]  [ "normal" ]  ]  , runAnim [ floor ( getFrames (  )  / 4 )  % runAnim . len (  )  ]  , charx + gvScreenW / 2 , gvScreenH / 2 )  ; 
  ...
  if ( game . playerChar2 != 0 && game . playerChar2 != "" ) drawSprite ( sprSulphurNimbus , SulphurNimbus . an [ "fly" ]  [ wrap ( getFrames (  )  / 4 , 0 , 7 )  ]  ,  ( game . hasSulphur == 1 ? charx :  - charx )  + gvScreenW / 2 , gvScreenH / 2 - 32 )  ; 
 
  else drawSprite ( sprSulphurNimbus , SulphurNimbus . an [ "fly" ]  [ wrap ( getFrames (  )  / 4 , 0 , 7 )  ]  , gvScreenW / 2 , gvScreenH / 2 - 32 )  ; 
...
gvMap . drawTiles ( floor (  - camx )  , floor (  - camy )  , floor ( camx / 16 )  - 3 , floor ( camy / 16 )  ,  ( gvScreenW / 16 )  + 5 ,  ( gvScreenH / 16 )  + 2 , "mg" )  ; 
 ...
 drawSprite ( sprVoid , 0 , 0 +  ( i * 16 )  , gvMap . h - 32 - camy )  ; 
  ...
  {     var foreachOutput11 = squirrelForEach( actor );     while(true)     {        foreachOutput11.next();        if (foreachOutput11.isDone()) break; i = foreachOutput11.getValue();  if (  ( (i[ ( "draw" ) ] !== undefined) )  &&  squirrelTypeOf ( i . draw )  == "function" ) i . draw (  )  ; 
...
gvInfoStep = 0 ; 
  ...
  if ( config . nearbars )  { 
  if (  ! gvSplitScreen )  { 
  if ( gvPlayer ) drawFloatingStats ( gvPlayer . x - camx0 , gvPlayer . y - camy0 ,  ( 1.0 / game . maxHealth )  * game . ps . health ,  ( 1.0 / game . ps . maxStamina )  * game . ps . stamina ,  ( 1.0 / game . ps . maxEnergy )  * game . ps . energy )  ; 
 
  if ( gvPlayer2 ) drawFloatingStats ( gvPlayer2 . x - camx0 , gvPlayer2 . y - camy0 ,  ( 1.0 / game . maxHealth )  * game . ps2 . health ,  ( 1.0 / game . ps2 . maxStamina )  * game . ps2 . stamina ,  ( 1.0 / game . ps2 . maxEnergy )  * game . ps2 . energy )  ; 
 ...
 for (  var i = 0 ;
 i < game . maxHealth ; i ++  ) drawSprite ( sprMeterBack , 1 , 26 +  ( i * 2 )  , 8 )  ; 
drawSprite ( sprMeterBack , 2 , 26 +  ( 2 * game . maxHealth )  , 8 )  ; 
setDrawColor ( 0xf83810ff )  ; 
 if ( game . ps . health > 0 ) drawRec ( 26 , 10 ,  ( game . ps . health * 2.0 )  - 1.0 , 3 , true )  ; 
...
setDrawColor ( 0x1080b0ff )  ; 
 if ( game . ps . energy > 0 ) drawRec ( 26 , 18 ,  ( game . ps . energy * 4.0 )  - 1.0 , 3 , true )  ; 
...
setDrawColor ( 0x70a048ff )  ; 
 if ( game . ps . stamina > 0 ) drawRec ( 26 , 26 ,  ( game . ps . stamina * 4.0 )  - 1.0 , 3 , true )  ; 
 ...
 for (  var i = 0 ;
 i < game . maxHealth ; i ++  ) drawSprite ( sprMeterBack , 1 , 26 +  ( i * 2 )  , 36 )  ; 
drawSprite ( sprMeterBack , 2 , 26 +  ( 2 * game . maxHealth )  , 36 )  ; 
setDrawColor ( 0xf83810ff )  ; 
 if ( game . ps2 . health > 0 ) drawRec ( 26 , 38 ,  ( game . ps2 . health * 2.0 )  - 1.0 , 3 , true )  ; 
...
setDrawColor ( 0x1080b0ff )  ; 
 if ( game . ps2 . energy > 0 ) drawRec ( 26 , 46 ,  ( game . ps2 . energy * 4.0 )  - 1.0 , 3 , true )  ; 
...
setDrawColor ( 0x70a048ff )  ; 
 if ( game . ps2 . stamina > 0 ) drawRec ( 26 , 54 ,  ( game . ps2 . stamina * 4.0 )  - 1.0 , 3 , true )  ; 
 
  } 
  
  if ( gvBoss )  { 
  var fullhearts = floor ( game . bossHealth /  ( 4 * 30 )  )  ;
...
drawSprite ( sprSkull , 0 , gvScreenW - 26 , gvScreenH - 86 )  ; 
 ...
 i < 10 ; i ++  )  { 
  ...
  else  if ( i == fullhearts && game . bossHealth > 0 ) drawSprite ( sprBossHealth , floor ( game . bossHealth / 30 )  % 4 , gvScreenW - 23 , gvScreenH - 96 -  ( 8 * i )  )  ; 
...
game . maxEnemies ++  ; 
 ...
 break ;  case 114 : c = newActor ( PeterFlower , i . x + 8 , i . y - 8 , i . name )  ; 
 break ;  }  return c ;
  }  ; 
